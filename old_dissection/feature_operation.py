    ...
    def feature_extraction(self, model=None, memmap=True):
        ...
        with torch.no_grad():
            for batch_idx,batch in enumerate(loader.tensor_batches(bgr_mean=self.mean)):
                ...
                if settings.GPU:
                    input = input.cuda()
                input_var = input  #input_var = V(input,volatile=True)
            ...
            for concept_map in batch:
                ...
                for unit_id in range(units):
                    ...
                    if feature_map.max() > threshold[unit_id]:
                        mask = np.array(Image.fromarray(feature_map).resize(size=(concept_map['sh'], concept_map['sw'])))   #imresize(feature_map, (concept_map['sh'], concept_map['sw']), mode='F')
